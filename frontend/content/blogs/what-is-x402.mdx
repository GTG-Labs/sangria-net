...
Today, Sangria supports external clients paying USDC on-chain to merchants hosted on the platform. Sangria receives the on-chain payment, verifies settlement, and credits the merchant's fiat balance — so merchants never need to handle crypto directly.

This model gives merchants reliable interoperability while insulating them from unnecessary crypto complexity.
...
The next internet payment layer is not about bigger checkout pages. It's about money moving autonomously at the speed of of a HTTP request.
