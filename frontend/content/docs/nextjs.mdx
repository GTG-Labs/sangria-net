...
If the Sangria backend is unreachable, times out, or returns a 5xx, the SDK throws a `SangriaError`. Next.js App Router doesn't have global error middleware for route handlers, so wrap your route with the `withSangriaErrorHandler` helper:

```ts
import {
  fixedPrice,
  withSangriaErrorHandler,
} from "@sangria-sdk/core/nextjs";
...
Without it, a flaky Sangria backend would surface as an opaque `500 Internal Server Error` to the agent. With the wrapper, they get a clean `503`.
