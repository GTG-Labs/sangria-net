...
If the Sangria backend is unreachable, times out, or returns a 5xx, the SDK throws a `SangriaError`. Register an Express error handler to turn this into a graceful fallback — without one, the agent sees an opaque `500 Internal Server Error`.
